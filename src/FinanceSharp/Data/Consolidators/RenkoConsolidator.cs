...
namespace QuantConnect.Data.Consolidators {
    ...
    public class RenkoConsolidator : IDataConsolidator {
        ...
        private readonly Func<IBaseData, double> _selector;
        private readonly Func<IBaseData, double> _volumeSelector;
        ...
        /// <param name="evenBars">When true bar open/close will be a multiple of the barSize</param>
        public RenkoConsolidator(double barSize, Func<IBaseData, double> selector, Func<IBaseData, double> volumeSelector = null, bool evenBars = true) {
        ...
        // Used for unit tests
        internal RenkoBar OpenRenkoBar => new RenkoBar(_openOn, _closeOn, _barSize, _openRate, _highRate, _lowRate, _closeRate);

        private void Rising(IBaseData data) {
            double limit;

            while (_closeRate > (limit = _openRate + _barSize)) {
                var wicko = new RenkoBar(_openOn, _closeOn, _barSize, _openRate, limit, _lowRate, limit);
        ...
        private void Falling(IBaseData data) {
            ...
            while (_closeRate < (limit = _openRate - _barSize)) {
                var wicko = new RenkoBar(_openOn, _closeOn, _barSize, _openRate, _highRate, limit, limit);
        ...
        private void UpdateWicked(IBaseData data) {
            ...
            if (_firstTick) {
                ...
                if (_closeRate > _openRate) {
                    ...
                    if (_closeRate > limit) {
                        var wicko = new RenkoBar(_openOn, _closeOn, _barSize, _lastWicko.Open, limit, _lowRate, limit);
                    ...
                    if (_closeRate < limit) {
                        var wicko = new RenkoBar(_openOn, _closeOn, _barSize, _lastWicko.Open, _highRate, limit, limit);
        ...
        private void UpdateClassic(IBaseData data) {
            ...
            if (_currentBar == null) {
                ...
                _currentBar = new RenkoBar(data.Time, _barSize, open, volume);
            }
        }
    ...
    public class RenkoConsolidator<TInput> : RenkoConsolidator
        ...
        /// 	 not aggregate volume per bar.</param>
        /// <param name="evenBars">When true bar open/close will be a multiple of the barSize</param>
        public RenkoConsolidator(double barSize, Func<TInput, double> selector, Func<TInput, double> volumeSelector = null, bool evenBars = true)
            : base(barSize, x => selector((TInput) x), volumeSelector == null ? (Func<IBaseData, double>) null : x => volumeSelector((TInput) x), evenBars) { }
