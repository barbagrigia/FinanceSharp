...
namespace FinanceSharp.Tests.Indicators {
    [TestFixture]
    public class MaximumTests : CommonIndicatorTests<IndicatorValue> {
        ...
        public void ComputesCorrectly() {
            ...
            max.Update(reference.AddDays(1), 1d);
            Assert.AreEqual(1d, max.Current.Value);
            Assert.AreEqual(0, max.PeriodsSinceMaximum);

            max.Update(reference.AddDays(2), -1d);
            Assert.AreEqual(1d, max.Current.Value);
            Assert.AreEqual(1, max.PeriodsSinceMaximum);

            max.Update(reference.AddDays(3), 0d);
            Assert.AreEqual(1d, max.Current.Value);
            Assert.AreEqual(2, max.PeriodsSinceMaximum);

            max.Update(reference.AddDays(4), -2d);
            Assert.AreEqual(0d, max.Current.Value);
            Assert.AreEqual(1, max.PeriodsSinceMaximum);

            max.Update(reference.AddDays(5), -2d);
            Assert.AreEqual(0d, max.Current.Value);
            Assert.AreEqual(2, max.PeriodsSinceMaximum);
        }
        ...
        public void ResetsProperlyMaximum() {
            var max = new Maximum(3);
            max.Update(DateTime.Today, 1d);
            max.Update(DateTime.Today.AddSeconds(1), 2d);
            max.Update(DateTime.Today.AddSeconds(2), 1d);
