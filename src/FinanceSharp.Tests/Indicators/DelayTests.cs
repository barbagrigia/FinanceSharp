...
namespace FinanceSharp.Tests.Indicators {
    ...
    public class DelayTests {
        ...
        public void DelayOneRepeatsFirstInputValue() {
            ...
            var data = (Time: DateTime.UtcNow, Value: new IndicatorValue(1d));
            delay.Update(data.Time, data.Value);
            Assert.AreEqual(1d, delay.Current.Value);

            data = (DateTime.UtcNow.AddSeconds(1), new IndicatorValue(2d));
            delay.Update(data.Time, data.Value);
            Assert.AreEqual(1d, delay.Current.Value);

            data = (DateTime.UtcNow.AddSeconds(1), new IndicatorValue(2d));
            delay.Update(data.Time, data.Value);
            Assert.AreEqual(2d, delay.Current.Value);
        }
        ...
        private void TestDelayTakesPeriodPlus2UpdatesToEmitNonInitialPoint(int period) {
            ...
            for (var i = 0; i < period + 2; i++) {
                Assert.AreEqual(0d, delay.Current.Value);
                delay.Update(DateTime.Today.AddSeconds(i), new IndicatorValue(i));
            }
