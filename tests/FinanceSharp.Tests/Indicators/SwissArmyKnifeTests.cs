...
namespace FinanceSharp.Tests.Indicators {
    [TestFixture]
    public class SwissArmyKnifeTests : CommonIndicatorTests<IndicatorValue> {
        ...
        public void ResetsProperly() {
            ...
            Assert.AreNotEqual(0d, sak.Current.Value);
        ...
        private static void AssertResult(double expected, double actual, double variance) {
            System.Diagnostics.Debug.WriteLine(expected + "," + actual + "," + Math.Abs((double) expected - actual));
            Assert.IsTrue(Math.Abs((double) expected - actual) < variance);
        }
    }
}